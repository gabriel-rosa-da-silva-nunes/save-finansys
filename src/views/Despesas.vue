            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container fluid>
                        ...
                        <v-row>
                            <v-col sm="12">
                                <v-text-field clearable outlined type="date" label="Data de Pagamento" v-model="dataDespesa"></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col sm="12">
    ...
    methods: {
        salvar() {
            if (this.insertModal == 'incluir') {
                ...
                if (this.id != null) {
                    db.collection('despesas').doc(this.id).update({
                        valor: this.valor,
                        data: this.dataDespesa,
                        tipo: this.tipoDespesa
                    }).then(
        ...
        deletar(item) {
            if (confirm("Tem certeza?")) {
                db.collection('despesas').doc(item.id).delete().then(
                    () => {
